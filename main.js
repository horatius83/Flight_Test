...
function createActionScene(game) {
    var script = [{x: 5, t: 500, pattern: 'kamikaze'}];
    var attackPatterns = { 'kamikaze': function(t,o) {
        o.y = t - o.age;
	o.x = Math.sin(o.y) * game.width;
	return o;
    	}
    };
    // Sort script by time
    // For every frame, check to see if the elapsed time has exceeded the script-time
	...
	// Spawn enemies
	if(this.age % 20 == 0) {
	    var enemy = new Enemy(rand(game.width), -32, scene);
	    game.enemies.push(enemy);
	}

	// Handle player
	if(game.touched && game.frame % 3 == 0) {
	    var s = new PlayerShoot(player.x, player.y - 15.0, scene);
	}
